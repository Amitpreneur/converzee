window.userid="61f2c52b72f85725868478fc";window.myConversionKit_host="https://api.converzee.com";window.mck_rUrl="https://d257yxqteot439.cloudfront.net/static";window.mck_support_URL="https://api.converzee.com/";window.campaigns = "";
     ...
     if (!window.leadengagr_loaded) {
      ...
      leadengagr.initExitIntent = function(campid) {
       ...
       if (!leadengagr_ExitIntent) {
        ...
        script.innerHTML =
         script.innerHTML +
         'function addEvent(obj, evt, fn) {if (obj.addEventListener) {obj.addEventListener(evt, fn, false);} else if (obj.attachEvent) {obj.attachEvent("on" + evt, fn);}}addEvent(window, "load", function(e) {addEvent(document, "mouseout", function(e) {e = e ? e : window.event;var from = e.relatedTarget || e.toElement;if (!from || from.nodeName == "HTML") {if(modalShowd){toggleModal();modalShowd = false;}}});});';
        var head = document.getElementsByTagName("head")[0];
        head.appendChild(style);
        var body = document.getElementsByTagName("body")[0];
      ...
      leadengagr.initGeoRedirection = function(campid) {
       ...
       if (!leadengagr_geoScript) {
        var script = document.createElement("script");
        script.innerHTML =
         'function leadEngarge_gro_Redirection(onSuccess) { var xhttp = new XMLHttpRequest();xhttp.onreadystatechange = function() {if (this.readyState == 4 && this.status == 200) {onSuccess(xhttp.responseText);}};xhttp.open("GET", "https://ipapi.co/json/", true);xhttp.send();}';
